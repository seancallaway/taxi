...
class TestWebsocket:
    ...
    async def test_cannot_connect_anonymously(self, settings):
    ...
    async def test_can_join_driver_pool(self, settings):
        settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
        _, access = await create_user(
    ...
    async def test_request_trip(self, settings):
        ...
        await communicator.send_json_to({
            ...
            'data': {
                'pick_up_address': '123 Main Street',
                'drop_off_address': '456 Test Avenue',
                'rider': user.id,
            },
        })
        ...
        assert response_data['drop_off_address'] == '456 Test Avenue'
    ...
    async def test_driver_is_alerted_on_request(self, settings):
        settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
        ...
        response = await channel_layer.receive(channel='test_channel')
        response_data = response.get('data')
    ...
    async def test_trip_group_creation(self, settings):
        settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
        user, access = await create_user('test.user@example.com', 'pAssw0rd', 'rider')
    ...
    async def test_user_joins_trip_group_on_connect(self, settings):
        settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
        user, access = await create_user('test.user@example.com', 'pAssw0rd', 'rider')
    ...
    async def test_driver_joins_trip_group_on_connect(self, settings):
        settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
        driver, access = await create_user('test.driver@example.com', 'pAssw0rd', 'driver')
